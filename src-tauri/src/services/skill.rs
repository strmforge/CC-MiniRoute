...
//!
//! v3.10.0+ 统一管理架构：
//! - SSOT（单一事实源）：`~/.cc-miniroute/skills/`
...
pub enum SkillStorageLocation {
    /// CC Switch 管理目录 (~/.cc-miniroute/skills/)
    #[default]
    CcSwitch,
...
impl SkillService {
    ...
    /// 获取 SSOT 目录（根据设置返回 ~/.cc-miniroute/skills/ 或 ~/.agents/skills/）
    ...
    /// 获取 Skill 卸载备份目录（~/.cc-miniroute/skill-backups/）
    ...
    fn resolve_uninstall_backup_source(skill: &InstalledSkill) -> Result<Option<PathBuf>> {
        // 返回值会被整目录复制进 ~/.cc-miniroute/skill-backups/ 并由 get_skill_backups
