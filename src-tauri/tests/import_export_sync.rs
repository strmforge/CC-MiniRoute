...
fn sync_codex_provider_writes_config_without_touching_auth() {
    ...
    let auth_path = cc_miniroute_lib::get_codex_auth_path();
    let config_path = cc_miniroute_lib::get_codex_config_path();
...
requires_openai_auth = true
"#;
    cc_miniroute_lib::write_codex_live_atomic(&legacy_auth, Some(legacy_config))
...
"#
    ...
    let toml_text =
        fs::read_to_string(cc_miniroute_lib::get_codex_config_path()).expect("read config.toml");
...
fn sync_enabled_to_codex_writes_enabled_servers() {
    ...
    let path = cc_miniroute_lib::get_codex_config_path();
    ...
    cc_miniroute_lib::sync_enabled_to_codex(&config).expect("sync codex");
...
fn sync_enabled_to_codex_returns_error_on_invalid_toml() {
    ...
    let err = cc_miniroute_lib::sync_enabled_to_codex(&config).expect_err("sync should fail");
    match err {
        cc_miniroute_lib::AppError::Toml { path, .. } => {
            assert!(
                path.ends_with("config.toml"),
                "path should reference config.toml"
            );
        }
        cc_miniroute_lib::AppError::McpValidation(msg) => {
            assert!(
...
fn sync_single_server_to_codex_fails_closed_on_invalid_toml() {
    ...
    let err = cc_miniroute_lib::sync_single_server_to_codex(
        &config,
        "srv",
...
fn sync_codex_provider_missing_auth_returns_error() {
    ...
    match err {
        cc_miniroute_lib::AppError::Config(msg) => {
    ...
    // 确认未产生任何 live 配置文件
    assert!(
        !cc_miniroute_lib::get_codex_auth_path().exists(),
        "auth.json should not be created on failure"
    );
    assert!(
        !cc_miniroute_lib::get_codex_config_path().exists(),
...
"#;
    ...
    cc_miniroute_lib::write_codex_live_atomic(&auth, Some(config_text))
    ...
    let stored_auth: serde_json::Value =
        cc_miniroute_lib::read_json_file(&auth_path).expect("read auth");
...
"#;
    ...
    let err = cc_miniroute_lib::write_codex_live_atomic(&auth, Some(config_text))
        .expect_err("config write should fail when target is directory");
    match err {
        cc_miniroute_lib::AppError::Io { path, .. } => {
            assert!(
                path.ends_with("config.toml"),
                "io error path should point to config.toml"
            );
        }
        cc_miniroute_lib::AppError::IoContext { context, .. } => {
            assert!(
...
"#,
    ...
    let mut config = MultiAppConfig::default();
    let changed = cc_miniroute_lib::import_from_codex(&mut config).expect("import codex");
...
fn sync_claude_enabled_mcp_projects_to_user_config() {
    ...
    cc_miniroute_lib::sync_enabled_to_claude(&config).expect("sync Claude MCP");

    let claude_path = cc_miniroute_lib::get_claude_mcp_path();
...
fn import_from_claude_merges_into_config() {
    ...
    let changed = cc_miniroute_lib::import_from_claude(&mut config).expect("import from claude");
...
fn create_backup_skips_missing_file() {
    ...
    reset_test_fs();
    let home = ensure_test_home();
    let config_path = home.join(".cc-miniroute").join("config.json");
...
fn create_backup_generates_snapshot_file() {
    ...
    reset_test_fs();
    let home = ensure_test_home();
    let config_dir = home.join(".cc-miniroute");
...
fn sync_gemini_packycode_sets_security_selected_type() {
    ...
    // security field is written to ~/.gemini/settings.json, not ~/.cc-miniroute/settings.json
